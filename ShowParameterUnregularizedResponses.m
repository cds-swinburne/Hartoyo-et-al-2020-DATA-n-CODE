function ShowParameterUnregularizedResponses(fparam)
...
for j=1:9
    ...
    for i=fparam.sorted_idx
        count = count + 1;
        mn = fparam.unregularized_response_statistics(i,j,1);
        mns(count) = mn;
        q1 = fparam.unregularized_response_statistics(i,j,3);
        q2 = fparam.unregularized_response_statistics(i,j,4);
    ...
    plot([0 0.32], [0 fparam.unregularized_parameter_response_gradient_statistics(j,1)*0.32], '-b', 'LineWidth', 1.5);
    hold on;
    ...
    TextLocation(['d', labl{j},'/dD_{JS} = ', num2str(fparam.unregularized_parameter_response_gradient_statistics(j,1),2), ' \pm ' , num2str(fparam.unregularized_parameter_response_gradient_statistics(j,2),2)],'Location','best')
    ylim([-1 1]);
